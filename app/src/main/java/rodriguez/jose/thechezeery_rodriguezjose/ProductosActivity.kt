...
class ProductosActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val tituloLogo: ImageView = findViewById(R.id.tituloLogo)
        val category = intent.getStringExtra("category")

        val products = when (category) {
            "cold" ->{
                tituloLogo.setImageResource(R.drawable.colddrinks)
                coldDrinks}
            "hot" -> {
                tituloLogo.setImageResource(R.drawable.hotdrinks)
                hotDrinks }
            "sweet" -> {
                tituloLogo.setImageResource(R.drawable.sweets)
                sweets}
            "salty" -> {
                tituloLogo.setImageResource(R.drawable.salties)
                salties}
            else -> ArrayList()
        }

        var adapter: ProductAdapter = ProductAdapter(this, products)
    ...
    fun addProduct() {
        ...
        coldDrinks.add(Product("Chocolate Frap", R.drawable.chocolatefrap, "CRich mocha-flavored sauce meets up with chocolaty chips, milk and ice for a blender bash.", 6))
    ...
    private class ProductAdapter: BaseAdapter{
        ...
        override fun getView(
            ...
            var desc = view.findViewById(R.id.prouctoDesc) as TextView
